...
def test_get_api_key_for_valid_user(email=valid_email, password=valid_password):
    """Тест получения API ключа для зарегистрированного пользователя"""
    status, result = pf.get_api_key(email, password)
    assert status == 200
    assert 'key' in result

def test_get_all_pets_with_valid_key(filter=''):
    """Тест получения не пустого списка питомцев по фильтру Все"""
...
def test_get_my_pets_with_valid_key(filter='my_pets'):
    """Тест получения не пустого списка питомцев по фильтру Мои"""
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.get_list_of_pets(auth_key, filter)
    assert status == 200
    assert len(result['pets']) > 0
...
def test_add_new_pet_with_valid_data(name='Матюся', animal_type='британец', age='9', pet_photo='images/cat11.jpg'):
    """Тест добавления нового питомца с корректными данными"""
...
def test_successful_update_self_pet_info(name='Матюсище', animal_type='двортерьер', age=6):
    """Тест возможности обновления информации о питомце"""
